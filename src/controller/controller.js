...
exports.getAddressBalance = async (req, res) => {
    try{
        ...
        // const balanceInUsd2Decimal = balanceInUsd.toFixed(2) + " USD";
        const balanceInUsd2Decimal = balanceInEth.toFixed(5) + " ETH";
...
exports.getTransactionDeatils = async (req, res) => {
    try{
        ...
        transactions.forEach(transaction => {
            ...
            var eth = value / 10**18; // Convert from Wei to Ether
            eth = parseFloat(eth.toFixed(5));
            if (eth <= 0)
            {
                eth = "1 wei"
            }
...
exports.getEthereumPrice = async (req, res) => {
    try {
        const response = await axios.get('https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd');
        const ethereumPrice = response.data.ethereum.usd + " USD";
